...
class KubeCtl:
    ...
    def __init__(self):
        ...
        # Determine kubeconfig file path explicitly
        ...
        try:
            if cluster_env:
                ...
                # Explicitly specify kubeconfig file path to avoid path resolution issues
                config.load_kube_config(config_file=kubeconfig_path)
        except config.ConfigException as e:
            raise RuntimeError(
                f"Failed to load Kubernetes configuration from {kubeconfig_path}: {e}\n"
                f"Please check:\n"
                f"  1. kubectl is working: try 'kubectl cluster-info'\n"
                f"  2. Kubeconfig file exists: {kubeconfig_path}\n"
                f"  3. KUBECONFIG environment variable is set correctly (if using custom path)\n"
                f"  4. Kubernetes cluster is accessible"
            )
        except Exception as e:
            # Catch any other exceptions (like FileNotFoundError)
            error_type = type(e).__name__
            raise RuntimeError(
                f"Unexpected error loading Kubernetes configuration from {kubeconfig_path}: {error_type}: {e}\n"
                f"Please check:\n"
                f"  1. kubectl is working: try 'kubectl cluster-info'\n"
                f"  2. Kubeconfig file exists and is readable: {kubeconfig_path}\n"
                f"  3. File permissions are correct"
            )
        
        # Create API clients - this is where the actual connection might fail
        try:
            ...
            error_type = type(e).__name__
            raise RuntimeError(
                f"Failed to create Kubernetes API clients: {error_type}: {e}\n"
                f"This usually indicates a problem with:\n"
                f"  1. API server connectivity (check: kubectl cluster-info)\n"
                f"  2. Certificate/authentication issues in kubeconfig\n"
                f"  3. Network connectivity to the API server"
            )
